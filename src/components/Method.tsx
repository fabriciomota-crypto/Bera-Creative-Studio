import React from 'react';
import methodContent from '../content/method.json';
import { useContent } from '../content/useContent';
import { Reveal } from './Reveal';

export const Method: React.FC = () => {
  const t = useContent(methodContent);

  return (
    <section className="py-section bg-surface/50">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <Reveal>
          <h2 className="text-h2 font-heading text-text mb-16 max-w-[30ch]">{t.title}</h2>
        </Reveal>

        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-x-8 gap-y-12">
          {t.steps.map((step) => (
            <Reveal key={step.number} className="border-t-2 border-grey/20 pt-8">
              <span className="block text-5xl font-heading font-light text-accent/40 mb-4">{step.number}</span>
              <h3 className="text-h3 font-heading text-text mb-3">{step.title}</h3>
              <p className="text-textMuted leading-relaxed">{step.description}</p>
            </Reveal>
          ))}
        </div>
      </div>
    </section>
  );
};
